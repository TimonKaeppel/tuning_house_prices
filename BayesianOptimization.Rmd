...
## Train Test Split, the training data will be split into folds for cv

```{r}
# do 5f-cv on every tuning step! Don`t tune one time on the whole fold splits.
...
tuning_results <- BayesianOptimization(
  ...
  init_points = 10,  # Number of initial points to sample
...
```{r}
# merge the timings with the tuning results, before gotta do some adjustments in the df u know..
