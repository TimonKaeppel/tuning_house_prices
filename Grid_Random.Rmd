...
# Cross-Validation plus Tuning with Random Search all parameters
...
# Create an empty list to store the fold indices
fold_indices <- vector("list", k)

# Calculate the number of observations in each fold
fold_size <- floor(nrow(house_prices) / k)
...
for (i in 1:k) {
  ...
  # For the last fold, adjust the end index to include any remaining observations
  if (i == k) {
    end_index <- nrow(house_prices)
  }
  
  fold_indices[[i]] <- start_index:end_index
}
...
for (i in 1:k) {
  ...
  train_set <- house_prices[train_indices, ]
  test_set <- house_prices[test_indices, ]
...
# Remove the first empty row from the results frame
results_frame <- results_frame[-1, ]

```

```{r}
write.csv(results_frame, file = "./resources/results_all_random/results_all_random.csv")
```
